...
// SQL query to fetch staff from PULAU TIOMAN/ROMPIN/TG.GEMOK department
$sql = "SELECT firstName, lastName, workposition, icnumber, email, phoneNumber, department 
        FROM jkdmregister 
        WHERE department = 'PULAU TIOMAN/ROMPIN/TG.GEMOK'
        ORDER BY department, 
            CASE 
...
<body>
    <h2>PULAU TIOMAN/ROMPIN/TG.GEMOK Staff List</h2>
    <table>
        <thead>
            <tr>
                <th>First Name</th>
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<td>
                        <form action='updateStaff.php' method='post'>
                            <input type='hidden' name='icnumber' value='" . $row["icnumber"] . "'>
            ...
            echo "<tr><td colspan='8' class='error-message'>No records found</td></tr>";
        }
        $conn->close();
        ?>
        </tbody>
